...
// 🔹 BARU: Tipe untuk Penanggung Jawab berdasarkan Shift (sesuai route.ts)
type ResponsibleStaffByShift = {
    [key in ShiftKey]: Staff | null;
}
...
// 🔹 RESPONSIBLE STAFF PERUBAHAN: Mengganti responsibleStaff tunggal
...
const createEmptyAssignment = (): ShiftAssignment => ({
  ...
  responsibleStaffByShift: DEFAULT_RESPONSIBLE_STAFF, // 🔹 Default struktur baru
});
...
function useArchiveShift() {
  ...
  // 🔹 RESPONSIBLE STAFF PERBAIKAN: Payload kini menggunakan responsibleStaffByShift
  const archive = useCallback(async (payload: ShiftAssignment | null) => {
    setIsLoading(true);
    
    // Mengecek apakah ada staf yang ditugaskan di salah satu shift atau anestesi
    const hasNurseAssignment = Object.values(payload?.nurseTeams || {}).some(team => team.length > 0);
    const hasAnesthesia = (payload?.anesthesiaTeam || []).length > 0;
    const hasPJ = Object.values(payload?.responsibleStaffByShift || DEFAULT_RESPONSIBLE_STAFF).some(pj => pj !== null);

    if (
      !payload ||
      (!hasNurseAssignment && !hasAnesthesia && !hasPJ)
    ) {
...
function useSaveShift() {
  ...
  // 🔹 RESPONSIBLE STAFF PERUBAHAN: Menerima ShiftAssignment dengan responsibleStaffByShift
  const saveShift = useCallback(async (newAssignment: ShiftAssignment) => {
    setIsSaving(true);
    const payload = {
        anesthesiaTeam: newAssignment.anesthesiaTeam,
        nurseTeams: newAssignment.nurseTeams,
        responsibleStaffByShift: newAssignment.responsibleStaffByShift, // 🔹 Pastikan payload dikirim dengan nama field yang benar
    }
...
/* ============================
   Modal: AssignRoomModal
   ============================ */
interface AssignRoomModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (newTeam: ActiveStaff[]) => void;
  roomName: string;
  availableStaff: ActiveStaff[];
  initialTeam: ActiveStaff[];
  isSaving: boolean;
}
...
// 🔹 RESPONSIBLE STAFF PERUBAHAN: Komponen list untuk memilih 1 staf penanggung jawab
const ResponsibleStaffList: React.FC<{
  staffList: Staff[];
  selectedStaff: Staff | null;
  onSelect: (staff: Staff) => void;
  onClear: () => void; // Tombol untuk menghapus PJ
  isSaving: boolean;
  searchTerm: string;
}> = ({ staffList, selectedStaff, onSelect, onClear, isSaving, searchTerm }) => (
  <>
    {selectedStaff && (
        <div className="mb-3 p-3 bg-blue-50 dark:bg-blue-900/40 rounded-lg flex justify-between items-center border border-blue-200 dark:border-blue-700">
            <span className="font-semibold text-blue-900 dark:text-blue-100 flex items-center">
                ...
                PJ Saat Ini: {selectedStaff.name} ({selectedStaff.role})
            </span>
            <motion.button onClick={onClear} whileTap={{ scale: 0.9 }} className="text-xs text-red-600 dark:text-red-400 hover:text-red-700 disabled:opacity-50" disabled={isSaving}>
                [Hapus]
            </motion.button>
        </div>
    )}
          ...
          <motion.li key={st.id} variants={itemVariants} className="rounded-md">
            <label className="flex items-center p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer">
              <input
                type="radio"
                checked={isSelected}
                onChange={() => onSelect(st)}
                disabled={isSaving}
                name="responsibleStaff"
                className="h-4 w-4 rounded-full text-blue-600 border-slate-300 dark:border-slate-600 focus:ring-blue-500"
              />
              <span className={`ml-3 font-medium ${isSelected ? 'text-blue-600 dark:text-blue-400' : 'text-slate-700 dark:text-slate-100'}`}>{st.name}</span>
              <span className="ml-2 text-xs text-slate-500 dark:text-slate-400">({st.role})</span>
            </label>
          </motion.li>
        );
...
const AssignShiftModal: React.FC<AssignShiftModalProps> = ({ isOpen, onClose, onSave, allStaff, currentAssignment, isSaving }) => {
  ...
  // 🔹 RESPONSIBLE STAFF PERUBAHAN: State untuk Penanggung Jawab per Shift
  const [responsibleStaffs, setResponsibleStaffs] = useState<ResponsibleStaffByShift>(DEFAULT_RESPONSIBLE_STAFF);

  const [mainTab, setMainTab] = useState<'pj' | 'anestesi' | 'perawat'>('pj');
  const [nurseTab, setNurseTab] = useState<ShiftKey>('Pagi');
  const [pjShiftTab, setPjShiftTab] = useState<ShiftKey>('Pagi'); // 🔹 Tab untuk navigasi PJ
  const [searchTerm, setSearchTerm] = useState('');

  // Memisahkan daftar staf
  const { doctorAnesthesiaStaff, nurseStaff, generalStaff } = useMemo(() => {
    const lcRole = (s: Staff) => s.role.toLowerCase();
    const isAnesthesiologist = (s: Staff) => lcRole(s).includes('dokter') && lcRole(s).includes('anestesi');
    const isNurse = (s: Staff) => lcRole(s).includes('perawat');

    return {
      doctorAnesthesiaStaff: allStaff.filter(isAnesthesiologist),
      nurseStaff: allStaff.filter(isNurse),
      // 🔹 RESPONSIBLE STAFF PERUBAHAN: Daftar PJ bisa dari semua staf
      generalStaff: allStaff, 
    };
  }, [allStaff]);
  ...
  useEffect(() => {
    if (isOpen) {
      if (currentAssignment) {
        ...
        // 🔹 PERBAIKAN: Set PJ dari struktur map baru
        setResponsibleStaffs(currentAssignment.responsibleStaffByShift || DEFAULT_RESPONSIBLE_STAFF); 
      } else {
        setAnesthesiaIds(new Set());
        setPagiIds(new Set());
        setSiangIds(new Set());
        setMalamIds(new Set());
        setResponsibleStaffs(DEFAULT_RESPONSIBLE_STAFF); // 🔹 Reset PJ dengan struktur default
      }
      setSearchTerm('');
      setMainTab('pj');
      setNurseTab('Pagi');
  ...
  // --- Logika Toggle Tim Perawat/Anestesi (Tidak Diubah) ---
  ...
  // 🔹 RESPONSIBLE STAFF PERUBAHAN: Handler select dan clear PJ
  const handleSelectResponsibleStaff = (staff: Staff) => {
    setResponsibleStaffs(prev => ({
      ...prev,
      [pjShiftTab]: staff
    }));
  };
  
  const handleClearResponsibleStaff = () => {
    setResponsibleStaffs(prev => ({
      ...prev,
      [pjShiftTab]: null
    }));
  ...
  const handleSaveClick = async () => {
    const newAssignment: ShiftAssignment = {
      ...
      responsibleStaffByShift: responsibleStaffs, // 🔹 Gunakan struktur map baru
    };
  ...
  const filteredGeneralStaff = generalStaff.filter((s) => s.name.toLowerCase().includes(term)); // 🔹 Filter untuk PJ
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="bg-white dark:bg-slate-900 rounded-lg shadow-xl w-full max-w-2xl p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b border-slate-200 dark:border-slate-700 mb-4 gap-4">
          <div className="flex">
            {/* 🔹 RESPONSIBLE STAFF PERUBAHAN: Tab untuk Penanggung Jawab */}
            <TabButton label="Penanggung Jawab" isActive={mainTab === 'pj'} onClick={() => setMainTab('pj')} />
        ...
        <AnimatePresence mode="wait">
          {/* 🔹 RESPONSIBLE STAFF PERUBAHAN: Konten Tab Penanggung Jawab */}
          {mainTab === 'pj' && (
            <motion.div key="pj" variants={tabContentVariants} initial="hidden" animate="visible" exit="exit">
              <div className="flex gap-2 mb-4 p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                {SHIFTS.map((shift) => (
                    <SubTabButton key={shift} label={shift} isActive={pjShiftTab === shift} onClick={() => setPjShiftTab(shift)} />
                ))}
              </div>

              <ResponsibleStaffList
                staffList={filteredGeneralStaff}
                selectedStaff={responsibleStaffs[pjShiftTab]}
                onSelect={handleSelectResponsibleStaff}
                onClear={handleClearResponsibleStaff}
                isSaving={isSaving}
...
const ShiftDisplay: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 🔹 RESPONSIBLE STAFF PERBAIKAN: Mengambil responsibleStaffByShift
      setAssignment({
  ...
  // Kumpulan staf untuk modal kamar (SEMUA staf dinas)
      ...
      <div className="space-y-2">
        {groupedStaff.map(([duty, staffList]) => (
          <div key={duty}>
            ...
            <motion.ul className="space-y-1 pl-2" variants={listVariants} initial="hidden" animate="visible">
              {staffList.map((st) => (
                <motion.li
                  key={st.id}
          ...
          {!totalLoading && !error && assignment && (
            <motion.div key="content" className="grid grid-cols-1 lg:grid-cols-3 gap-6" variants={mainContentVariants} initial="hidden" animate="visible" exit="exit">
              ...
              <motion.div className="lg:col-span-1 space-y-6" variants={sectionVariants}>
                ...
                {/* 🔹 RESPONSIBLE STAFF PERBAIKAN: Tampilkan Penanggung Jawab OK Harian (List) */}
                <motion.div className="p-4 bg-yellow-50 dark:bg-yellow-900/40 border-l-4 border-yellow-400 dark:border-yellow-600 rounded-r-lg" variants={sectionVariants}>
                  <h3 className="font-semibold text-yellow-800 dark:text-yellow-200 mb-3 flex items-center text-lg"><CheckCircle size={18} className="mr-2.5" />Penanggung Jawab OK Harian</h3>
                  
                  <div className="space-y-3">
                    {SHIFTS.map((shiftKey) => {
                      const pjStaff = assignment.responsibleStaffByShift[shiftKey];
                      ...
                      return (
                        ...
                        >
                          ...
                          {pjStaff ? (
                            ...
                            <p className="text-xs text-slate-500 italic pl-4">Belum ditetapkan.</p>
                          )}
                        </div>
                      );
                    })}
                ...
                {/* 🔹 Akhir dari Penanggung Jawab OK Harian */}
