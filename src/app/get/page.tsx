...
import {
  ...
  UserPlus,
  Search,
  CheckCircle, // 🔹 Icon baru untuk Penanggung Jawab
} from 'lucide-react';
...
// 🔹 RESPONSIBLE STAFF PERUBAHAN: Tambahkan responsibleStaff
interface ShiftAssignment {
  anesthesiaTeam: Staff[];
  nurseTeams: NurseShiftTeams;
  responsibleStaff?: Staff; // Bisa null/undefined jika belum diatur
}
...
const createEmptyAssignment = (): ShiftAssignment => ({
  ...
  responsibleStaff: undefined, // 🔹 Default undefined
});
...
/* ============================
   Utilities
   ============================ */
// Fungsi getCurrentShift DIBIARKAN DIKOMENTARI (seperti di kode sebelumnya)
...
function useArchiveShift() {
  ...
  // 🔹 RESPONSIBLE STAFF PERUBAHAN: Memeriksa responsibleStaff saat archive
...
function useSaveShift() {
  ...
  // 🔹 RESPONSIBLE STAFF PERUBAHAN: Menerima ShiftAssignment dengan responsibleStaff
...
// ... (StaffCheckboxList, TabButton, SubTabButton tidak diubah) ...
...
const ResponsibleStaffList: React.FC<{
  ...
  isSaving: boolean;
  searchTerm: string;
}> = ({ staffList, selectedId, onSelect, isSaving, searchTerm }) => (
  <motion.ul
    className="h-64 overflow-y-auto space-y-1 p-2 bg-slate-50 dark:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700"
    variants={listVariants}
    initial="hidden"
...
const AssignShiftModal: React.FC<AssignShiftModalProps> = ({ isOpen, onClose, onSave, allStaff, currentAssignment, isSaving }) => {
  ...
  // 🔹 RESPONSIBLE STAFF PERUBAHAN: State untuk Penanggung Jawab
  const [responsibleStaff, setResponsibleStaff] = useState<Staff | null>(null);

  const [mainTab, setMainTab] = useState<'pj' | 'anestesi' | 'perawat'>('pj'); // 🔹 Default tab PJ
  const [nurseTab, setNurseTab] = useState<ShiftKey>('Pagi');
  const [searchTerm, setSearchTerm] = useState('');

  // Memisahkan daftar staf
  const { doctorAnesthesiaStaff, nurseStaff, nonAnesthesiaStaff } = useMemo(() => {
    ...
    return {
      ...
      // 🔹 RESPONSIBLE STAFF PERUBAHAN: Daftar PJ bisa dari semua staf (non-anestesi/perawat juga)
      nonAnesthesiaStaff: allStaff.filter(s => !isAnesthesiologist(s)), 
    };
  }, [allStaff]);
  ...
  useEffect(() => {
    if (isOpen) {
      if (currentAssignment) {
        ...
        setResponsibleStaff(currentAssignment.responsibleStaff || null); // 🔹 Set PJ
      } else {
        setAnesthesiaIds(new Set());
        setPagiIds(new Set());
        setSiangIds(new Set());
        setMalamIds(new Set());
        setResponsibleStaff(null); // 🔹 Reset PJ
      }
      setSearchTerm('');
      setMainTab('pj'); // 🔹 Default ke tab PJ
      setNurseTab('Pagi');
    }
  ...
  const handleSaveClick = async () => {
    const newAssignment: ShiftAssignment = {
      ...
      responsibleStaff: responsibleStaff || undefined, // 🔹 Tambahkan PJ
    };
  ...
  const filteredResponsibleStaff = nonAnesthesiaStaff.filter((s) => s.name.toLowerCase().includes(term)); // 🔹 Filter untuk PJ
          ...
          {mainTab === 'pj' && (
            <motion.div key="pj" variants={tabContentVariants} initial="hidden" animate="visible" exit="exit">
              <ResponsibleStaffList
                staffList={filteredResponsibleStaff}
                selectedId={responsibleStaff?.id || null}
...
const ShiftDisplay: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 🔹 RESPONSIBLE STAFF PERUBAHAN: Pastikan responsibleStaff ada di setAssignment
      setAssignment({
          ...
          {!totalLoading && !error && assignment && (
            <motion.div key="content" className="grid grid-cols-1 lg:grid-cols-3 gap-6" variants={mainContentVariants} initial="hidden" animate="visible" exit="exit">
              ...
              <motion.div className="lg:col-span-1 space-y-6" variants={sectionVariants}>
                ...
                {/* 🔹 RESPONSIBLE STAFF PERUBAHAN: Tampilkan Penanggung Jawab */}
                <motion.div className="p-4 bg-yellow-50 dark:bg-yellow-900/40 border-l-4 border-yellow-400 dark:border-yellow-600 rounded-r-lg" variants={sectionVariants}>
                  <h3 className="font-semibold text-yellow-800 dark:text-yellow-200 mb-3 flex items-center text-lg"><CheckCircle size={18} className="mr-2.5" />Penanggung Jawab OK</h3>
                  {assignment.responsibleStaff ? (
                    <div className="flex items-center text-sm p-1.5 rounded-lg bg-yellow-100 dark:bg-yellow-800">
                        <User size={14} className="mr-2 text-yellow-600 flex-shrink-0" />
                        <span className="font-bold text-yellow-900 dark:text-yellow-100">{assignment.responsibleStaff.name}</span>
                        <span className="text-yellow-700 dark:text-yellow-300 ml-1.5">({assignment.responsibleStaff.role})</span>
                    </div>
                  ) : (
                    <p className="text-sm text-slate-500 italic">Belum ada penanggung jawab ditetapkan.</p>
                  )}
                </motion.div>
