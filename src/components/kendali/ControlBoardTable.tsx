...
export default function ControlBoardTable({ schedule, onCallPatient, onAssignOR, onAssignTeam }: ControlBoardTableProps) {
  return (
          ...
          {schedule.map((surgery) => (
            <tr key={surgery.id} className="dark:hover:bg-gray-800">
              ...
              <td className="p-3">
                ...
                <button onClick={() => onAssignOR(surgery)} className="ml-2 text-xs text-blue-500 hover:underline">[Atur]</button>
              </td>
              <td className="p-3">
                {surgery.assignedTeam?.nurses.length || 0} Perawat
                <button onClick={() => onAssignTeam(surgery)} className="ml-2 text-xs text-blue-500 hover:underline">[Atur]</button>
              </td>
