...
// Sub-komponen untuk menampilkan baris detail agar lebih rapi
...
export default function SurgeryDetailModal({ surgery, onClose }: SurgeryDetailModalProps) {
  const statusConfig: Record<ScheduledSurgery['status'], 'blue' | 'green' | 'red' | 'yellow' | 'indigo' > = {
    'Terjadwal': 'blue',
    'Terkonfirmasi': 'green',
    'Dibatalkan': 'red',
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl relative animate-fade-in-down">
        ...
        <header className="border-b pb-4 mb-4">
          <h2 className="text-2xl font-bold text-gray-800">Detail Jadwal Operasi</h2>
          <div className="flex items-center gap-4 mt-2">
            <p className="text-gray-500">ID: {surgery.id}</p>
            <Badge colorScheme={statusConfig[surgery.status]}>{surgery.status}</Badge>
          </div>
        </header>

        <main className="divide-y">
            <DetailRow icon={<User size={20} />} label="Nama Pasien" value={surgery.patientName} />
            <DetailRow icon={<ClipboardList size={20} />} label="Nomor Rekam Medis" value={surgery.mrn} />
            <DetailRow icon={<ClipboardList size={20} />} label="Prosedur Direncanakan" value={surgery.procedure} />
            <DetailRow icon={<Stethoscope size={20} />} label="Dokter Penanggung Jawab" value={surgery.doctorName} />
            <DetailRow 
